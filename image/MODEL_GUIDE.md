...
## ⚙️ Motor de Inferencia (v6.7 RAW)

Para garantizar la estabilidad en Apple Silicon, el servidor utiliza una conexión directa al API de Ollama (Puerto 11434):

* **Aislamiento:** No depende de librerías externas que filtren metadatos (PNG real garantizado).
* **Persistencia:** Los archivos se guardan en `/image/outputs/` con un ID de tarea único vinculado al log.
* **Sincronismo:** El endpoint `/generate` bloquea al cliente hasta que la imagen existe físicamente en disco.

mi forma de crear los servidores locales tiene esta arquitectura, este ejemplo es muy adecuado pues se trata de un modelo post entrenado pero para emitir texto+coordenadas en formato de texto natural. 
la arquitectura comprende : capa de script puente entre cliente servidor (, despierta al servidor , chakehands, administración de formateo estrategia de ingesta), servidor (con salida en bruto, carga de modelo, sistema de autoapagado por times de tiempo sin actividad, descarga de modelo, alertas sonoras de carga y descarga. ), capa de script entre server y cliente que administra la salida del server y el formato del cliente :

ejemplos de caso similar :
